...
[courtyard.io](https://courtyard.io/) — pay a fixed price, get a random
number of cards drawn from a finite inventory.
...
- **Three price points**:
  | Price | Cards per pull |
  |-------|-----------------|
  | $50   | 1 card |
  | $100  | 3 cards |
  | $500  | 20 cards (best value per card) |
- **5,000-card inventory**, `cards.js` generates placeholders `Card #0001`
  ...
  Pulls are weighted by those percentages, and each card can only be
  pulled once — pulling depletes the shared inventory until all 5,000
  are gone ("Sold Out").
...
- **State is per-browser**, stored in `localStorage`
  (`vault-pulls-state-v1`). Two different browsers/devices each see a
